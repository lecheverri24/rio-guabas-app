<nav class="bg-emerald-900 text-white">
    <div class="w-full mx-auto px-4 sm:px-6 lg:px-8">
        <div class="flex justify-between h-16 items-center">
            ...
            <div class="flex-shrink-0">
                <a href="/home" class="text-xl font-bold hover:text-emerald-300 transition-colors">
                    Rio Guabas
                </a>
            </div>
            ...
            <div class="hidden md:flex space-x-6">
                ...
                @foreach ($items as $item)
                    <a href="{{ route($item['route']) }}"
                       class="relative hover:text-emerald-300 transition-colors 
    ...
    <div id="mobile-menu" class="hidden md:hidden px-2 pt-2 pb-3 space-y-1">
        @foreach ($items as $item)
            <a href="{{ route($item['route']) }}"
               class="block px-3 py-2 rounded hover:bg-emerald-800 {{ request()->routeIs($item['route']) ? 'bg-emerald-900' : '' }}">
                {{ $item['label'] }}
            </a>
        @endforeach

        @guest
            <a href="{{ route('login') }}" class="block px-3 py-2 rounded hover:bg-emerald-800">Iniciar sesión</a>
            <a href="{{ route('register') }}" class="block px-3 py-2 rounded hover:bg-emerald-800">Registrarse</a>
        @endguest

        @auth
            <a href="{{ route('settings.profile') }}" class="block px-3 py-2 rounded hover:bg-emerald-800">Perfil</a>
            <form method="POST" action="{{ route('logout') }}">
                @csrf
                <button type="submit" class="w-full text-left px-3 py-2 rounded hover:bg-emerald-800">
                    Cerrar sesión
                </button>
            </form>
        @endauth
